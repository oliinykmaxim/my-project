...
import type { CheckoutRequestDto, CreateCheckoutRequestDto, ApiError } from "./dtos.js";
...
const tbody = document.getElementById('itemsTableBody') as HTMLElement | null;
...
async function loadTable() {
    ...
    try {
        const responseData: any = await getRequests();
...
// Малювання таблиці з типізованими даними
// Малювання таблиці з типізованими даними
function renderTable(items: CheckoutRequestDto[]) {
    if (!tbody) return;
    
    tbody.innerHTML = items.map((item: any, index) => {
        ...
        if (savedStatus) {
            ...
            if (item.status === 'active') displayStatus = 'Approved';
            else if (item.status === 'maintenance') displayStatus = 'Maintenance';
        }
...
function readForm(): any {
    const statusSelect = (document.getElementById('statusSelect') as HTMLSelectElement).value;
    const itemCode = (document.getElementById('itemCodeInput') as HTMLInputElement).value.trim();
    ...
    return {
        itemCode: itemCode,
        userName: (document.getElementById('userInput') as HTMLInputElement).value.trim(),
        dateFrom: (document.getElementById('dateFromInput') as HTMLInputElement).value,
        dateTo: (document.getElementById('dateToInput') as HTMLInputElement).value,
...
// Валідація
function validate(dto: any): boolean {
    clearErrors();
    let isValid = true;
    if (dto.itemCode === "") {
...
function showError(inputId: string, errorId: string, message: string) {
...
function clearErrors() {
    ...
    errors.forEach(error => { (error as HTMLElement).innerHTML = ''; });
}
...
if (form) {
    form.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            alert(`Не вдалося створити запис: ${(error as ApiError).message}`);
        } finally {
...
(window as any).handleDelete = async function(id: number) {
